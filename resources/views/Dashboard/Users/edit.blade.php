            ...
            <form method="POST" action="{{ route('users.update', $user->id) }}">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" required 
                           ...
                           value="{{ old('name', $user->name) }}" placeholder="{{ __('text.enter_name_placeholder') }}">
                ...
                <div class="mb-6">
                    ...
                    <input type="email" name="email" id="email" required 
                           ...
                           value="{{ old('email', $user->email) }}" placeholder="{{ __('text.enter_email_placeholder') }}">
                    @error('email') <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p> @enderror
                </div>
                
                {{-- حقل كلمة المرور (اختياري) --}}
                <div class="mb-6">
                    <label for="password" class="block text-gray-700 font-semibold mb-2">{{ __('text.password') }} ({{ __('text.optional') }})</label>
                    <input type="password" name="password" id="password"
                           ...
                           placeholder="{{ __('text.leave_blank_for_no_change') }}">
                ...
                <div class="mb-8">
                    ...
                    <select name="role" id="role" required
                        ...
                        @foreach ($roles as $name => $displayName)
                            <option value="{{ $name }}" 
                                        @selected(old('role', $user->roles->first()->name ?? null) === $name)> 
                                {{ $displayName }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex justify-end">
                    <button type="submit" 
                        ...
                        @if (app()->isLocale('ar'))
                            <i class="fas fa-save ms-2"></i> {{ __('text.update_user_btn') }}
                        @else
                            <i class="fas fa-save me-2"></i> {{ __('text.update_user_btn') }}
                        @endif
                    </button>
                </div>
            </form>
